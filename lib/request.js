...
module.exports = {
  ...
  get host() {
    var trust = this.app.proxy;
    var host = this.get('X-Forwarded-Host');

    if (!host || !trust(this.socket.remoteAddress)) {
      host = this.get('Host');
    }

    // To Do: IPv6 literal support
    return host;
  },
  ...
  get protocol() {
    var trust = this.app.proxy;
    if (!trust(this.socket.remoteAddress)) {
  ...
  get ip() {
    return proxyaddr(this.req, this.app.proxy);
  },
  ...
  get ips() {
    return proxyaddr.all(this.req, this.app.proxy).slice(1).reverse();
  },
